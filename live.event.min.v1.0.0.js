...
this.index++}}
this.fireEvent=()=>{this.cbNow()}}};delete(eventNames,cb){var check=eventNames.replace(/ /g,"");var have_multiple=check.split(",");var one_callback=1;var inst=have_multiple.map((env,inx)=>{if(this._instance[env])delete this._instance[env];if(this.watchInstance[env])delete this.watchInstance[env];if(!this.watchInstance[env]&&!this._instance[env]&&one_callback===1){console.debug(`eventName/watchInstance ${env} deleted!`)
...
return}
var check=eventName.replace(/ /g,"");var have_multiple=check.split(",");var _t=this;var inst=have_multiple.map((env,inx)=>{if(!env){console.debug('no match for ',eventName)
return}
...
return inst}
watch(eventName,cb){if(!eventName){console.debug('watch, need to provice eventName');return!1}
setTimeout(()=>{var evn;this.callEvent(eventName,(d)=>{if(!d.eventName){return}
...
dispatch(eventName,data){if(!eventName){console.debug('dispatch, need to provide eventName');return}
